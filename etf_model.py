...
load_file_directory(os.path.join("ETF Data", "train"), in_train_set = True)
load_file_directory(os.path.join("ETF Data", "test"), in_train_set = False)
...
#conv branch 2
conv_1_2 = Conv1D(32, 15, padding='same', kernel_regularizer=regularizers.l2(0.0025), bias_regularizer=regularizers.l2(0.0025))(input_layer)
conv_2_2 = Conv1D(64, 15, padding='same', kernel_regularizer=regularizers.l2(0.0025), bias_regularizer=regularizers.l2(0.0025))(conv_1_2)
max_pool_1_2 = MaxPool1D(pool_size=15, padding='same')(conv_2_2)
conv_3_2 = Conv1D(128, 15, padding='same', kernel_regularizer=regularizers.l2(0.0025), bias_regularizer=regularizers.l2(0.0025))(max_pool_1_2)
conv_4_2 = Conv1D(256, 15, padding='same', kernel_regularizer=regularizers.l2(0.0025), bias_regularizer=regularizers.l2(0.0025))(conv_3_2)
...
#conv branch 3
conv_1_3 = Conv1D(32, 25, padding='same', kernel_regularizer=regularizers.l2(0.0025), bias_regularizer=regularizers.l2(0.0025))(input_layer)
max_pool_1_3 = MaxPool1D(pool_size=25, padding='same')(conv_1_3)
conv_2_3 = Conv1D(64, 25, padding='same', kernel_regularizer=regularizers.l2(0.0025), bias_regularizer=regularizers.l2(0.0025))(max_pool_1_3)
...
output_layer = Dense(250, activation=activations.linear, kernel_regularizer=regularizers.l2(0.0025), bias_regularizer=regularizers.l2(0.0025))(dropout_2)

network = Model(input_layer, output_layer)
network.compile(loss=losses.MAPE)
plot_model(network, to_file='etf_network.png', show_shapes=True)
print(network.summary())
callbacks_list_etfs = [callbacks.EarlyStopping(monitor='val_loss', patience=1,), callbacks.ModelCheckpoint(filepath='etf_predictor.h5', monitor='val_loss', save_best_only=True,), callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1,patience=10,), callbacks.TensorBoard(log_dir='etf_tensorboard',histogram_freq=1,embeddings_freq=1,)]
history = network.fit(X_train, y_train, epochs=100, batch_size=128, validation_split=0.2, callbacks=callbacks_list_etfs)
network = load_model('etf_predictor.h5')
...
os.mkdir(os.path.join("ETF Data", "predictions"))

for idx in range(num_rows):
    data = {"prediction": predictions[idx], "actual": y_test[idx]}
    data = pd.DataFrame.from_dict(data)
    filename = str(idx) + ".csv"
    filename = os.path.join("ETF Data", "predictions", filename)
